  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#e0e0e0] p-4 sm:p-6">
        ...
        {/* Modo Offline de Nómina (Paso 1 & Paso 2) */}
        <div className={`border p-4 rounded-xl mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all ${
          offlineNomina
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold text-xs sm:text-sm">
                🔒 Modo Offline de Nómina (Anti-Reinicio por Señal)
              </span>
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-wider ${
                offlineNomina ? 'bg-amber-400 text-amber-950' : 'bg-gray-200 text-gray-700'
              }`}>
                {offlineNomina ? 'Nómina Fija Bloqueada' : 'Sincronización Abierta'}
              </span>
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                ...
                <span>{isOnline ? 'Conectado a Internet' : 'Sin Señal de Red'}</span>
              </span>
            </div>
            <p className="text-xs mt-0.5 max-w-lg opacity-90">
              {offlineNomina
                ? 'Los trabajadores cargados a nóminas quedan bloqueados localmente. Al perder o recuperar señal, no se re-sincronizarán ni restablecerán.'
                : 'Permite sincronizar y actualizar la lista de trabajadores con la nube o el servidor.'}
            </p>
          </div>
          ...
          {onToggleOfflineNomina && (
            ...
            >
              ...
              <span>{offlineNomina ? 'MODO OFFLINE ACTIVO' : 'ACTIVAR MODO OFFLINE'}</span>
            </button>
          )}
        </div>
